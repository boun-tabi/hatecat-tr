...
def build_trainer(
    ...
    output_dir = f"{output_dir_prefix}_train"
    ...
    split_ds = tokenized_dataset["train"].train_test_split(test_size=eval_train_split_ratio, seed=42)
...
def parse_args():
    ...
    p.add_argument("--eval_train_split_ratio", type=int, default=0.1, help="Fraction of training data to use for evaluation.")

    # Output
    p.add_argument("--output_prefix", type=str, default="runs",
                   help="Prefix for output directories. Final dirs: <output_prefix>_{all|full}.")
...
def main():
    ...
    print("Text:", tokenized_dataset["test"]["Text"][ix] if "Text" in tokenized_dataset["test"].column_names else "<no Text column>")

    gold_stream = [
        tok.convert_ids_to_tokens(token) if label in range(1, len(id2label)) else "-"
        for label, token in zip(tokenized_dataset["test"]["labels"][ix], tokenized_dataset["test"]["input_ids"][ix])
    ]
    pred_stream = [
        tok.convert_ids_to_tokens(token) if label in range(1, len(id2label)) else "-"
        for label, token in zip(np.argmax(predictions[ix], axis=1)[: len(tokenized_dataset["test"]["input_ids"][ix])],
                                tokenized_dataset["test"]["input_ids"][ix])
    ]
    print("Gold tags token stream:", "".join(gold_stream))
    print("Pred  tags token stream:", " ".join(pred_stream).replace(" ##", ""))
