...
df_full = pd.read_csv(f'{base_path}/full_match.csv')
...
for annotator in annotators:
    file_path = f'{base_path}/annotators/{annotator}/{annotator.capitalize()}.csv'
    if annotator == 'irem':
        file_path = f'{base_path}/annotators/{annotator}/İrem.csv'
...
# Updated label mappings for categorization
id2label = {
    0: "O",
...
for dataset in ds.keys():
    ...
    model = AutoModelForTokenClassification.from_pretrained(
        ...
        num_labels=6  # Updated for 6 categories including 'O'
    )
    data_collator = DataCollatorForTokenClassification(tokenizer=tokenizer)

    wandb.init(
        project="categorization_annotator",
    ...
    print('Label:', ''.join([
        tokenizer.convert_ids_to_tokens(token) if label in list(range(1,6)) else '-' 
    ...
    prediction_text = ' '.join([
        ...
        for label, token in zip(
            np.argmax(predictions[ix], axis=1)[:len(tokenized_dataset["test"]['input_ids'][ix])],
